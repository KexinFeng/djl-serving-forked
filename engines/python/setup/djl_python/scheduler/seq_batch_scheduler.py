...
class SeqBatchScheduler:
    ...
    def __init__(self, default_lm_block: LMBlock, default_seq_batcher_cls, default_config: SearchConfig):
        self.default_search_configs = defaultdict(lambda: default_config)
        self.default_seq_batcher_cls = default_seq_batcher_cls
        self.lm_block = default_lm_block
        self.results = {}

        self.seq_batcher = None  # {key: seq_batcher}

    def add_request(self, input_ids: torch.Tensor, request_uids: torch.Tensor, lm_block: LMBlock = None,
                    seq_batcher_cls=None, search_configs: List[SearchConfig] = None,
                    kv_cache: Union[Tuple, None] = None, save_kv_cache_path: str = None):
        ...
        if search_configs:
            for request, search_config in zip(request_uids.view(-1).tolist(),
                                              search_configs):
                self.default_search_configs[request] = search_config

        lm_block_seq = self.lm_block if lm_block is None else lm_block
        seq_batcher_cls = self.default_seq_batcher_cls if seq_batcher_cls is None else seq_batcher_cls

        # prefill
        new_seq_batcher, output_ids = seq_batcher_cls.init_forward(input_ids=input_ids, request_uids=request_uids,
                                                                   lm_block=lm_block_seq,
                                                                   search_configs=self.default_search_configs,
                                                                   kv_cache=kv_cache, save_kv_cache_path=save_kv_cache_path)
        ...
        for request_uid, output_id in zip(request_uids.view(-1).tolist(),
                                          output_ids):
