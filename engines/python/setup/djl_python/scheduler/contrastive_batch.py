...
class ContrastiveBatch(Batch):
    ...
    def __init__(self, seq_dim_order: List[int], past_output_ids: torch.Tensor = None,
                 past_attention_mask: torch.Tensor = None, past_hidden_states: torch.Tensor = None,
                 logits: torch.Tensor = None, past_key_values: List[torch.Tensor] = None):
        super().__init__(seq_dim_order, past_output_ids, past_attention_mask, logits, past_key_values)
