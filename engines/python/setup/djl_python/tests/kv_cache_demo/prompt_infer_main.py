...
def main(args):
    ...
    for idx, (prompt_id, batch_size, cache_method) in enumerate(itertools.product(prompt_ids, batch_sizes,
        ...
        if cache_method == file_cache:
            ...
            r = cache_method(kv_cache_files[0], input_str, request_vol=batch_size, reps=reps)
            print((r[0], (r[1]-r[0])/20))
            result[idx], result_tot[idx] = r
        else:
            r = cache_method(prompt_str, input_str, request_vol=batch_size, reps=reps)
            print((r[0], (r[1]-r[0])/20))
            result[idx], result_tot[idx] = r
